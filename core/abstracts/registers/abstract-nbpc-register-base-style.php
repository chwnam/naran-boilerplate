 ...
 *
 * abstracts/registers/abstract-nbpc-register-base-style.php
 */
...
if ( ! class_exists( 'NBPC_Register_Base_Style' ) ) {
	abstract class NBPC_Register_Base_Style implements NBPC_Register {
		...
		protected function src_helper( string $rel_path, bool $replace_min = true ): string {
			...
			if ( $replace_min && nbpc_script_debug() && str_ends_with( $rel_path, '.min.css' ) ) {
			...
			if ( nbpc_is_theme() ) {
				...
				return plugin_dir_url( nbpc_main_file() ) . 'assets/css/' . $rel_path;
			}
		}
	}
}
